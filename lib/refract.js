...
function refract(src, method) {
    ...
    if (typeof src === 'string') {
        ...
        if (!fs.existsSync(path.resolve(config.modules[src]['bin'], `${src}.js`))) {
            ...
            if (!fs.existsSync(path.resolve(config.modules[src]['bin'], `${src}.wasm`)))
                throw Error('Failed to build ' + src);
            if (!fs.existsSync(path.resolve(config.modules[src]['bin'], `${src}.js`)) &&
                config.modules[src]['gluecode_src']) {
                execSync(`cp ${config.modules[src]['gluecode_src']} ${config.modules[src]['bin']}/${src}.js`);
            }
            else
                throw Error('No gluecode located in bin');
        }
        // Load it
